...
//        String s = HttpClientUtils.doGet("http://172.217.24.74/maps/api/js?key=AIzaSyDidcF9DFtw26wHzdUla4IlIqv8xsLbhsw", null, null);
//        System.out.println(s.length());
        String newUrl = "https://maps.googleapis.com/maps/api/js?key=AIzaSyDidcF9DFtw26wHzdUla4IlIqv8xsLbhsw&sensor&libraries=visualization&language=en-US";
        String[] cmds={"curl",  "--socks5", "127.0.0.1:10808", newUrl};
        String s = execCurl(cmds);
        System.out.println(s.length());
        String jsContent = IOUtils.toString(URI.create(newUrl));
        System.out.println(jsContent.length());
        if(true && false || true ){
